		...
		int migratetype, fpi_t fpi_flags)
{
	...
	/* delete all page cap */
	if(cslot_allocator_init && seL4_buddy_root_init_flag) { 
		if(order == MAX_ORDER) {
			tail_page_cslot = sel4_get_page_cslot_id(page);
			if(tail_page_cslot > INVALID_CAP_ID) {
				page_cslot = seL4_CNode_Delete(tail_page_cslot);
				sel4_set_page_cslot_id(page, page_cslot);
			}
		}
	}
	...
	while (order < MAX_PAGE_ORDER) {
		...
		if(cslot_allocator_init && seL4_buddy_root_init_flag) {
			...
			tail_page_cslot = sel4_get_page_cslot_id(buddy);
			if(tail_page_cslot > INVALID_CAP_ID) {
				page_cslot = seL4_CNode_Delete(tail_page_cslot);
			}
		}
				  ...
				  int high, int migratetype)
{
	...
	while (high > low) {
		...
		/* new feature for cap system in linux */
		if(cslot_allocator_init && seL4_buddy_root_init_flag) {
			unsigned long src_cslot_id, dest_cslot_id;

			/* get merge cap id */
			src_cslot_id = sel4_get_page_cslot_id(page);

			/* alloc cap for right page */
		    dest_cslot_id = sel4_cslot_alloc();
			if (dest_cslot_id <= 0 || dest_cslot_id >= MAX_CSLOTS) {
				pr_err("ZhuangL error: %s >>>>>>>>>>>>>>> Failed to allocate capability for page\n", __func__);
				sel4_set_page_cslot_id(&page[size], INVALID_CAP_ID);
			} else {
				sel4_set_page_cslot_id(&page[size], dest_cslot_id);
				/* perform the first retype */
				seL4_Untype_Retype(src_cslot_id, dest_cslot_id, OBJ_UNTYPED,
					PAGE_SIZE * size, &page[size]);
			}
			...
			if (dest_cslot_id <= 0 || dest_cslot_id >= MAX_CSLOTS) {
				...
				sel4_set_page_cslot_id(page, dest_cslot_id);
...
out:
	...
	/* new feature for cap system in linux */
	if(page && cslot_allocator_init && seL4_buddy_root_init_flag) {
			int i;
			unsigned long cslot_id, src_cslot_id;

			src_cslot_id = sel4_get_page_cslot_id(page);
			if(src_cslot_id > INVALID_CAP_ID) {
				for (i = 0; i < (1 << order); i++) {
					cslot_id = sel4_cslot_alloc();
					if (cslot_id <= 0 || cslot_id > MAX_CSLOTS) {
							pr_err("ZhuangL error: %s >>>>>>>>>>>>>>> Failed to allocate capability for page\n", __func__);
							sel4_set_page_cslot_id(page + i, INVALID_CAP_ID);
						} else {
							sel4_set_page_cslot_id(page + i, cslot_id);
					}
					/* each page should retype from untyped cap to page cap */
					seL4_Untype_Retype(src_cslot_id, cslot_id, OBJ_PAGE, PAGE_SIZE, (page + i));
				}
			}
	}
